...
export default function CompleteButton({ isCompleted, modifyTask } : CompleteButtonProps ) {
    ...
    return (
        <>
            <div className={`${!theme && "bg-darkBlue"} rounded-full flex items-center justify-center ${isCompleted ? "bg-gradient-to-br from-cyan-300 to-purple-500" : (theme ? "border border-gray-400" : "border border-darkLine")} w-[20px] h-[20px]`} onClick={() => modifyTask()}>
                {isCompleted && (
